...
export const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, onClose, onConfirm, amount, team, targetValue }) => {
  ...
  const handlePay = async (provider: 'venmo' | 'cashapp') => {
    ...
    if (provider === 'venmo') {
      window.open(`https://venmo.com/?txn=pay&amount=${amount}&note=LipidLottoBet`, '_blank');
    } else {
  ...
  const teamNameDisplay = team === 'TALLOW' 
    ? "Team Nước Béo (Fatty Broth)" 
    : "Team Khổ Qua (Bitter Melon)";

  const teamColorClass = team === 'TALLOW' ? 'text-red-500' : 'text-emerald-500';

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-[#151525] w-full max-w-md rounded-2xl border border-slate-700 overflow-hidden shadow-2xl transform transition-all relative">
        
        {/* Header */}
        <div className="p-5 border-b border-slate-700 flex justify-between items-start bg-slate-900/50">
          <div>
            <h3 className="text-white font-black text-lg tracking-wide uppercase">Payment Required</h3>
            <p className="text-xs text-slate-400 font-mono">THE GREASE TAX</p>
          </div>
        ...
        <div className="p-6">
          <div className="text-center mb-8 relative">
             ...
             <div className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-2">Total Due</div>
             <div className="text-5xl font-black text-white mb-2 drop-shadow-lg">${amount.toFixed(2)}</div>
             
             <div className="bg-slate-800/50 rounded-lg p-3 inline-block border border-slate-700 mt-2">
               <div className="text-xs text-slate-400 mb-1">Your Target Selection</div>
               <div className={`text-sm font-bold uppercase ${teamColorClass}`}>
                 {teamNameDisplay}
               </div>
          ...
          <div className="mt-6 p-3 bg-red-900/20 border border-red-900/50 rounded-lg text-center">
            <p className="text-[10px] text-red-300 italic font-medium leading-relaxed">
              "By paying, you acknowledge that I am eating a burger right now with your money."
            </p>
          </div>
        </div>
        ...
        {processing && (
          <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm">
            <div className="flex flex-col items-center">
              <div className="w-8 h-8 border-4 border-[#00e676] border-t-transparent rounded-full animate-spin mb-4"></div>
              <div className="text-[#00e676] font-bold text-sm tracking-widest animate-pulse">PROCESSING LARD...</div>
            </div>
          </div>
        )}
